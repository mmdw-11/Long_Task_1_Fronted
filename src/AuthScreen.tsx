...
export function AuthScreen({api,onAuthenticated}:{api:ApiClient;onAuthenticated:(value:AuthResult)=>void}){
 const [mode,setMode]=useState<Mode>('login'); const [busy,setBusy]=useState(false); const [error,setError]=useState(''); const [notice,setNotice]=useState(''); const [resetToken,setResetToken]=useState(''); const [resetEmail,setResetEmail]=useState('');
 const submit=async(e:React.FormEvent<HTMLFormElement>)=>{e.preventDefault();setBusy(true);setError('');setNotice('');const f=new FormData(e.currentTarget);try{
  ...
  if(mode==='forgot'){const result=await authApi.post<{message:string;reset_token?:string}>('/api/auth/forgot-password',{email:f.get('email')});setResetEmail(String(f.get('email')||''));setResetToken(result.reset_token||'');setNotice(result.message);setMode('reset');return}
 ...
 return <main className="auth-page"><section className="auth-hero"><div className="auth-brand"><img src="/brand-logo.jpg" alt="TIDE-Swarm 标识"/><b>TIDE-Swarm</b></div><div><em>AI AGENT OPERATIONS</em><h1>把复杂任务，交给一支<br/>可观察的智能体团队。</h1><p>设计编排、授权工具、实时追踪执行，并保留完整、可审计的任务结果。</p></div><footer>安全会话 · 结构化事件 · 可控工具权限</footer></section><section className="auth-panel"><div className="auth-card"><small>AGENTFORGE CONSOLE</small><h2>{mode==='login'?'欢迎回来':mode==='register'?'创建账户':mode==='forgot'?'找回密码':'设置新密码'}</h2><p>{mode==='login'?'登录后进入智能体运行平台':mode==='register'?'注册后将自动登录':mode==='forgot'?'输入注册邮箱获取验证码':`验证码已发送至 ${resetEmail || '你的邮箱'}，30 分钟内有效`}</p>{error&&<div className="auth-message error">{error}</div>}{notice&&<div className="auth-message">{notice}</div>}<form onSubmit={submit}>
  ...
  {mode==='reset'&&<label><span>邮箱验证码</span><input name="token" required defaultValue={resetToken} inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" placeholder="请输入邮件中的 6 位验证码"/></label>}
 ...
 </form><div className="auth-links">{mode==='login'?<><button onClick={()=>change('forgot')}>忘记密码？</button><span>还没有账户？<button onClick={()=>change('register')}>立即注册</button></span></>:<button onClick={()=>change('login')}>← 返回登录</button>}</div></div></section></main>
}

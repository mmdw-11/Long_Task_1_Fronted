...
const blank = {name:'',provider:'openai-compatible',model_id:'',base_url:'',api_key:'',tier:'cloud' as Tier};
...
export function ModelConnectionPanel({api,models,presets,done}:{api:ApiClient;models:ModelConnection[];presets:Preset[];done:()=>unknown}) {
  const [query,setQuery]=useState(''),[tier,setTier]=useState(''),[status,setStatus]=useState('');
  ...
  const open=(model:ModelConnection|null)=>{opener.current=document.activeElement as HTMLElement;const next=model?{name:model.name,provider:model.provider,model_id:model.model_id,base_url:model.base_url,api_key:'',tier:model.tier}:{...blank};setEditing(model);setForm(next);setInitial(JSON.stringify(next));setClearKey(false);setError('');setOpened(true);};
  ...
  const save=()=>action('save',async()=>{const {api_key,...fields}=form;const payload={...fields,...(clearKey?{api_key:'',api_key_env:''}:api_key.trim()?{api_key:api_key.trim()}:{}),...(editing&&editing.tier!==form.tier?{auto_default:false}:{})};const saved=editing?await api.put<ModelConnection>(`/api/model-connections/${editing.id}`,payload):await api.post<ModelConnection>('/api/model-connections',payload);setOpened(false);setNotice(`“${saved.name}”已保存${saved.test_status==='succeeded'?'，原测试状态保留':'，请执行连接测试'}。`);});
  const missing=tiers.filter(([t])=>!models.some(m=>m.tier===t&&m.auto_default&&runnable(m))).map(([,label])=>label);
  const rows=models.filter(m=>(!tier||m.tier===tier)&&(!status||(status==='disabled'?!m.enabled:status==='ready'?runnable(m):m.enabled&&!runnable(m)))&&`${m.name} ${m.model_id} ${m.provider}`.toLowerCase().includes(query.toLowerCase()));
  return <div className="mm">
    ...
    <section className="mm-auto"><div className="mm-toolbar"><h3>自动调度配置</h3><span className={missing.length?'mm-warning':'mm-success'}>{missing.length?'未就绪':'已就绪'}</span></div>
      <div className="mm-tiers">{tiers.map(([t,label])=>{const current=models.find(m=>m.tier===t&&m.auto_default);return <label key={t}>{label}默认模型<select disabled={!!busy} value={current?.id||''} onChange={e=>{const id=e.target.value;void action('default',async()=>{if(id)await api.put(`/api/model-connections/${id}`,{auto_default:true});else if(current)await api.put(`/api/model-connections/${current.id}`,{auto_default:false});});}}><option value="">请选择已测试模型</option>{models.filter(m=>m.tier===t&&(runnable(m)||m.id===current?.id)).map(m=><option key={m.id} value={m.id} disabled={!runnable(m)}>{m.name}{!runnable(m)?'（不可用）':''}</option>)}</select></label>;})}</div>
    ...
    <div className="mm-filters"><input aria-label="搜索模型" placeholder="搜索连接名称、模型 ID、提供商" value={query} onChange={e=>setQuery(e.target.value)}/><select aria-label="层级筛选" value={tier} onChange={e=>setTier(e.target.value)}><option value="">全部层级</option>{tiers.map(([t,label])=><option key={t} value={t}>{label}</option>)}</select><select aria-label="状态筛选" value={status} onChange={e=>setStatus(e.target.value)}><option value="">全部状态</option><option value="ready">可用</option><option value="pending">待测试 / 测试失败</option><option value="disabled">已停用</option></select><span>{rows.length} 个连接</span></div>
    <div className="mm-table"><table><thead><tr><th>连接名称</th><th>模型 / 提供商</th><th>层级</th><th>状态 / 最近测试</th><th>AUTO</th><th>操作</th></tr></thead><tbody>{rows.map(m=><tr key={m.id}><td><button className="mm-name" onClick={()=>open(m)}>{m.name}</button></td><td><b>{m.model_id}</b><small>{m.provider}</small></td><td>{tiers.find(([t])=>t===m.tier)?.[1]}</td><td><span className={runnable(m)?'mm-success':'mm-warning'}>{!m.enabled?'已停用':m.test_status==='succeeded'?'测试成功':m.test_status==='failed'?'测试失败':'待测试'}</span>{results[m.id]&&<small className={results[m.id].ok?'':'mm-error'}>{results[m.id].text}<br/>{results[m.id].time}</small>}</td><td>{m.auto_default?'默认':'—'}</td><td><div className="mm-actions"><button disabled={!!busy} onClick={()=>open(m)}>编辑</button><button disabled={!!busy||!m.enabled} onClick={()=>test(m)}>{busy===m.id?'测试中…':'测试'}</button><select aria-label={`${m.name}的更多操作`} disabled={!!busy} value="" onChange={async e=>{const command=e.target.value;if(!command)return;if(!(await appConfirm(`${command==='delete'?'删除':'更改启用状态'}“${m.name}”？${m.auto_default?'该连接是 AUTO 默认模型，操作会影响自动调度。':''}已引用应用可能受影响，请先替换连接。`)))return;void action(m.id,()=>command==='delete'?api.delete(`/api/model-connections/${m.id}`):api.put(`/api/model-connections/${m.id}`,{enabled:!m.enabled}));}}><option value="">更多</option><option value="toggle">{m.enabled?'停用':'启用'}</option><option value="delete">删除</option></select></div></td></tr>)}</tbody></table>{!rows.length&&<p className="mm-empty">{models.length?'没有符合筛选条件的连接':'还没有模型连接，点击“添加模型”开始配置。'}</p>}</div>
    {opened&&<div className="mm-overlay" onMouseDown={e=>{if(e.target===e.currentTarget)close();}}><section className="mm-drawer" role="dialog" aria-modal="true" aria-labelledby="mm-title" onKeyDown={e=>{if(e.key!=='Tab')return;const items=Array.from(e.currentTarget.querySelectorAll<HTMLElement>('button:not(:disabled),input:not(:disabled),select:not(:disabled)'));const first=items[0],last=items[items.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last?.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first?.focus();}}}>
      ...
      <form onSubmit={e=>{e.preventDefault();void save();}}><div className="mm-form">
        ...
        <div className="mm-form-pair"><label>提供商<select value={form.provider} onChange={e=>{const provider=e.target.value;setForm(f=>({...f,provider,...(!editing?{base_url:presets.find(p=>p.provider===provider)?.base_url||''}:{})}));}}><option value="openai-compatible">自定义兼容服务</option>{!presets.some(p=>p.provider===form.provider)&&form.provider!=='openai-compatible'&&<option value={form.provider}>{form.provider}</option>}{presets.map(p=><option key={p.provider} value={p.provider}>{p.name}</option>)}</select></label><label>所属层级<select value={form.tier} onChange={e=>update('tier',e.target.value)}>{tiers.map(([t,label])=><option key={t} value={t}>{label}</option>)}</select></label></div>
        ...
        <p className="mm-tip">仅改名保留测试状态；修改模型 ID、地址或密钥后需重新测试。更改层级会取消原 AUTO 默认标记。密钥不回显，但当前仍保存在后端本地文件，请保护存储目录。</p>

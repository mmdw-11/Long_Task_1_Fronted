...
export function ResourcePage({ api, notify, kind }: { api: ApiClient; notify: Notify; kind: string }) {
  ...
  return <div className="resource-page"><div className="resource-head"><div><h2>{c.title}</h2><p>{c.hint}</p></div><button className="violet-btn" onClick={()=>setShow(true)}>＋ {c.create}</button></div><div className="resource-toolbar"><label>⌕<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜索名称、描述或 ID" /></label><button aria-label="刷新" title="刷新" onClick={q.load}>↻</button></div>{visibleItems.length?<div className="resource-grid">{visibleItems.map(item=><article key={item.id}><span className="resource-icon">{kind==='permissions'?'♙':kind==='data-connections'?'⌁':kind==='knowledge-bases'?'▤':'◈'}</span><h3>{item.name}</h3><p>{item.description||'尚未填写描述'}</p><small>状态：{item.status}　更新于 {new Date(item.updated_at).toLocaleString('zh-CN')}</small><button className="remove" onClick={async()=>{if((await appConfirm(`删除「${item.name}」？`))){try{await api.delete(`/api/resources/${kind}/${item.id}`);q.load();notify('已删除')}catch(e){notify((e as Error).message,true)}}}}>删除</button></article>)}</div>:!q.loading&&q.items.length?<div className="resource-empty"><b>没有匹配的结果</b><p>请尝试调整搜索关键词。</p></div>:!q.loading&&<div className="resource-empty"><b>{c.empty}</b><p>点击右上角“{c.create}”开始配置。</p></div>}{show&&<Editor title={c.create} name={name} description={description} setName={setName} setDescription={setDescription} close={()=>setShow(false)} submit={create}/>}</div>;
}

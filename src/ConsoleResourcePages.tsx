...
export function ResourcePage({ api, notify, kind }: { api: ApiClient; notify: Notify; kind: string }) {
  ...
  return <div className="resource-page"><div className="resource-head"><div><h2>{c.title}</h2><p>{c.hint}</p></div><button className="violet-btn" onClick={()=>setShow(true)}>＋ {c.create}</button></div><div className="resource-toolbar"><label>⌕<input placeholder="搜索名称、描述或 ID" /></label><button onClick={q.load}>↻</button></div>{q.items.length?<div className="resource-grid">{q.items.map(item=><article key={item.id}><span className="resource-icon">{kind==='permissions'?'♙':kind==='data-connections'?'⌁':kind==='knowledge-bases'?'▤':'◈'}</span><h3>{item.name}</h3><p>{item.description||'尚未填写描述'}</p><small>状态：{item.status}　更新于 {new Date(item.updated_at).toLocaleString('zh-CN')}</small><button className="remove" onClick={async()=>{if(confirm(`删除「${item.name}」？`)){try{await api.delete(`/api/resources/${kind}/${item.id}`);q.load();notify('已删除')}catch(e){notify((e as Error).message,true)}}}}>删除</button></article>)}</div>:!q.loading&&<div className="resource-empty"><b>{c.empty}</b><p>点击右上角“{c.create}”开始配置。</p></div>}{show&&<Editor title={c.create} name={name} description={description} setName={setName} setDescription={setDescription} close={()=>setShow(false)} submit={create}/>}</div>;
}
...
function Editor({title,name,description,setName,setDescription,close,submit}:{title:string;name:string;description:string;setName:(x:string)=>void;setDescription:(x:string)=>void;close:()=>void;submit:()=>void}){return <div className="overlay"><div className="modal"><div className="modal-head"><h2>{title}</h2><button onClick={close}>×</button></div><div className="form-grid"><label className="span">名称<input autoFocus value={name} onChange={e=>setName(e.target.value)} /></label><label className="span">描述<textarea rows={4} value={description} onChange={e=>setDescription(e.target.value)} /></label><div className="form-actions span"><button onClick={close}>取消</button><button className="primary" disabled={!name.trim()} onClick={submit}>确认创建</button></div></div></div></div>}

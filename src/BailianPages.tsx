...
export function MemoryBanks({ api, notify }: { api: ApiClient; notify: Notify }) {
  const q = useData<{ id: string; name: string; description: string; updated_at: string }>(api, '/api/memory-banks');
  const [name, setName] = useState(''); const [description, setDescription] = useState(''); const [creating, setCreating] = useState(false);
  const create = async () => { try { await api.post('/api/memory-banks', { name, description }); setName(''); setDescription(''); setCreating(false); q.load(); notify('记忆库已创建；它不会覆盖运行时上下文账本'); } catch (e) { notify((e as Error).message, true); } };
  return <ConsolePage title={`记忆库 ${q.data.length}`} action={<button className="violet-btn" onClick={() => setCreating(true)}>＋ 创建记忆库</button>}>
    <div className="info-banner">ⓘ 记忆库是可挂载的长期资料入口；现有 TODO、上下文防漂移、检查点机制仍由后端运行时独立维护。</div><div className="memory-search"><label className="market-search">⌕<input placeholder="请输入名称、描述或 ID" /></label><button className="icon-round" onClick={q.load}>↻</button></div>
    <div className="memory-cards">{q.data.map(bank => <article key={bank.id}><span>☁</span><h3>{bank.name}</h3><p>描述　{bank.description || '尚未填写描述'}</p><p>ID　 <code>{short(bank.id, 26)}</code></p><footer>更新于 {format(bank.updated_at)}</footer><button onClick={async () => { if (confirm('确认删除该记忆库目录？')) { await api.delete(`/api/memory-banks/${bank.id}`); q.load(); } }}>删除</button></article>)}</div>{!q.loading && !q.data.length && <Empty title="还没有记忆库" subtitle="创建一个记忆库后，可在后续应用配置中作为长期资料入口使用。" />}
